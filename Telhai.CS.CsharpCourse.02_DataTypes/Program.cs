...
class Program
{
    static void Main()
    {
        ...
        object boxVal = 100;
        int newBoxVal = (int)boxVal; // Not recomended
        if (boxVal is int valBox)   // Better
        {
            valBox += 100;
        }
        ...
        Func<int, int, int> add = (a, b) => a + b; // Last parm is return val of func
        Func<int, int, int> mult = Test;
        int resM = mult.Invoke(100,100);  // Invokes mult func
